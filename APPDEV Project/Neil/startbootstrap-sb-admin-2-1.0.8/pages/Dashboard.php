...
<body>
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header"> <div class="background-image"><form class="fonttext" style="font-size: 70px">Welcome, Vice President of Operations.</form></div>
                            ...
                            <div id="morris-area-chart"></div>
							 ...
							 <?php
                                foreach ($query as $row) {
                                    ...
                                    $temp = query("
 SELECT COUNT(MODULESNO) AS NUMBEROFEVENTS
   FROM PROJECTCHARTER PC JOIN MODULES M 
                            ON Pc.projectcharterid = M.projectcharterid 
  WHERE m.STATUSID=1 
    AND pc.projectCharterID=".$row['projectCharterID']
                                                    ,$conn);
                                    $temp2 = query("    
       SELECT COUNT(MODULESNO) AS NUMBEROFEVENTS
   FROM PROJECTCHARTER PC JOIN MODULES M 
    ...
    WHERE pc.projectCharterID=".$row['projectCharterID'],$conn);
                                    if( $temp2[0]['NUMBEROFEVENTS'] <> 0){
                                    $calculated = $temp[0]['NUMBEROFEVENTS']/$temp2[0]['NUMBEROFEVENTS']*100 ;
                                ...
                                '<div>
                                    <p>
                                        <a href="DashboardTaskList_backend.php?project='.$row['projectCharterID'].'"><strong style="font-size: 15px">'.$row['projectName'].'</strong></a>
                                        <span class="pull-right text-muted" style="font-size: 15px">'.substr($calculated, 0,strpos($calculated, '.')).'% In Progress</span>
                                    </p>
                                    <div class="progress progress-striped active">
                                        <div class="progress-bar progress-bar-success" role="progressbar" aria-valuenow="20" aria-valuemin="0" aria-valuemax="100" style="width: '.$calculated.'%">
                                            <span class="sr-only">'.$calculated.'% Complete</span>
                                        </div>
                                    </div>
                                </div>';
                                }   
